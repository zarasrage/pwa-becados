...
// Ordena becados por hace cuánto expusieron su último seminario (nunca = primero)
function ordenarPorEspera(becadosNombres, ultimoPorNombre, today) {
  return (becadosNombres || [])
    .map(nombre => {
      const ultima = ultimoPorNombre?.[nombre];
...
function SeminarioPicker({ existing, onSave, onDelete, onAplazar, onClose, becadosNombres, ultimoSemPorBecado, ultimoSemPorBecadoYTag, today, T }) {
  ...
  // Sugerencia según el tipo de seminario elegido (tag) — a quién le toca esa rotación
  const sugerenciasTag = useMemo(() =>
    ordenarPorEspera(becadosNombres, ultimoSemPorBecadoYTag?.[tag], today),
    [becadosNombres, ultimoSemPorBecadoYTag, tag, today]);

  // Sugerencia general — a quién le toca considerando seminarios de cualquier tipo
  const sugerenciasGeneral = useMemo(() =>
    ordenarPorEspera(becadosNombres, ultimoSemPorBecado, today),
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:200,display:"flex",flexDirection:"column",
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:T.surface,
        ...
        {/* Sugerencia según el tipo de seminario elegido */}
        {sugerenciasTag.length > 0 && (
          <div style={{marginBottom:10}}>
            <div style={{fontSize:12,fontWeight:600,color:T.muted,marginBottom:6}}>
              Sugerencia — {TAG_OPTS.find(t=>t.id===tag)?.label || tag} <span style={{fontWeight:400}}>(hace más tiempo sin exponer este tipo)</span>
            </div>
            <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:2}}>
              {sugerenciasTag.slice(0,6).map(({nombre,dias}, idx) => (
        ...
        {/* Sugerencia general — considerando seminarios de cualquier tipo */}
        {sugerenciasGeneral.length > 0 && (
          <div style={{marginBottom:10}}>
            <div style={{fontSize:12,fontWeight:600,color:T.muted,marginBottom:6}}>
              Sugerencia general <span style={{fontWeight:400}}>(todos los tipos de seminario)</span>
            </div>
            <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:2}}>
              {sugerenciasGeneral.slice(0,6).map(({nombre,dias}, idx) => (
                <button key={nombre} className="press" onClick={()=>setPresenter(nombre)}
                  style={{flexShrink:0,display:"flex",flexDirection:"column",alignItems:"center",gap:2,
                    padding:"6px 10px",borderRadius:10,cursor:"pointer",
                    border:`1.5px solid ${idx===0?"#4F6EFF":T.border}`,
                    background:idx===0?"#4F6EFF18":T.surface2}}>
                  <span style={{fontSize:12.5,fontWeight:600,color:idx===0?"#4F6EFF":T.text}}>{nombre}</span>
                  <span style={{fontSize:11,color:T.muted}}>{dias==null?"nunca":`hace ${dias}d`}</span>
                </button>
              ))}
            </div>
...
export function TabEditor({ onBack, allowedTipos, T }) {
  ...
  // Última fecha en que cada becado expuso un seminario (histórico completo), tanto en general
  // como por tipo (tag), para sugerir a quién le toca en el picker de seminarios.
  useEffect(() => {
    supabase.from("seminarios")
      .select("fecha, tag, presentador_nombre, becados(nombre)")
      .lte("fecha", today)
      .order("fecha", { ascending: false })
      .then(({ data }) => {
        const general = {}, porTag = {};
        for (const s of data || []) {
          const n = s.presentador_nombre || s.becados?.nombre;
          if (!n) continue;
          // orden desc: la primera aparición de cada nombre es la más reciente
          if (!general[n]) general[n] = s.fecha;
          if (s.tag) {
            if (!porTag[s.tag]) porTag[s.tag] = {};
            if (!porTag[s.tag][n]) porTag[s.tag][n] = s.fecha;
          }
        }
        setUltimoSemPorBecado(general);
        setUltimoSemPorBecadoYTag(porTag);
      });
  }, []);
