...
export const THEME_OPTIONS_BASIC = [
    ...
    <>
      ...
      <div style={{
              ...
              <button key={opt.id} className="press"
                ...
                style={{
                  ...
                  background: isActive
                    ...
                    : isHov ? `${opt.preview[0]}EE` : `${opt.preview[0]}CC`,
                  border:`1.5px solid ${isActive ? ac+"70" : isHov ? ac+"40" : ac+"18"}`,
                  borderRadius:18, padding:"13px 11px 11px", cursor:"pointer",
                  textAlign:"left", transition:"all 0.16s ease", overflow:"hidden",
                  boxShadow: isActive
                    ? `0 0 24px ${ac}25, inset 0 0 24px ${ac}08`
                    : isHov ? `0 4px 20px rgba(0,0,0,0.4), 0 0 12px ${ac}15` : "none",
                }}>
                {isActive && <div style={{position:"absolute",top:0,left:"15%",right:"15%",height:2,background:`linear-gradient(90deg,transparent,${ac},transparent)`,borderRadius:99}}/>}
                {isActive && <div style={{position:"absolute",top:9,left:9,width:7,height:7,borderRadius:"50%",background:ac,boxShadow:`0 0 10px ${ac}, 0 0 4px ${ac}`}}/>}
                ...
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                  ...
                  <span style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontSize:15,fontWeight:800,lineHeight:1,color:isActive?ac:isHov?"#E8F0FF":"#8090B0",transition:"color 0.15s"}}>{opt.name}</span>
                </div>
                <div style={{fontSize:10,lineHeight:1.4,paddingLeft:24,color:isActive?ac+"99":"#445060"}}>{opt.desc}</div>
              </button>
            );
          })}
        </div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,margin:"6px 0"}}>
          <div style={{flex:1,height:1,background:isDark?"#ffffff10":"#00000010"}}/>
          <span style={{fontSize:9,fontWeight:600,letterSpacing:"0.08em",color:isDark?"#334050":"#A0AEC0",textTransform:"uppercase"}}>más temas</span>
          <div style={{flex:1,height:1,background:isDark?"#ffffff10":"#00000010"}}/>
        </div>
